...
public class LoginController implements Initializable {
    ...
    public void performLogin()
    {
    	...
    	Packet packet = new Packet();
		packet.addCommand(Command.getUserByIdAndPass);
		
		ArrayList<Object> param = new ArrayList<>(Arrays.asList(user));
	
		packet.setParametersForCommand(Command.getUserByIdAndPass, param);
		...
		send.registerHandler(new IResultHandler() {
			...
			public void onReceivingResult(Packet p) {
				if (p.getResultState())
				{
					ArrayList<User> userList = p.<User>convertedResultListForCommand(Command.getUserByIdAndPass);
